...
//!
//! Bullet metrics use a two-level aggregation pattern:
//!   1. Inner subquery groups by (metric_key, person_id) and computes a per-
//!      person, per-period value (`v_period`) — sum for counters, avg for
//!      rates, max(0/1) for active indicators, custom formula for
//!      estimation_accuracy. OData's `metric_date` range filter is injected
//!      INSIDE this subquery (see analytics-api `inject_where_into_first_subquery`).
//!   2. Outer query aggregates across people with `avg`/`quantileExact(0.5)`/
//!      `min`/`max` → the bullet's value, median, range_min, range_max.
//!
//! Team bullets compare the team (filtered by `org_unit_id` via OData) to the
//! whole company (unfiltered `c` subquery). IC bullets compare a single person
//! to their team (JOIN on `org_unit_id`).
