...
| 92 | Bronze `tenant_id` column | 3.3, 3.4 | `tenant_id` added to `raw_people`, `raw_commits`, `raw_tasks` DDLs with ORDER BY |
...
| 99 | `raw_people.location` column | 3.3 | Added `location` to Bronze DDL matching bootstrap code and unified fields summary |
...
| 102 | Bronze `tenant_id` completeness | 3.4, 3.15 | `tenant_id` added to `raw_messages`, `raw_deals`, `raw_calendar`, `raw_activity` DDLs |
...
CREATE TABLE silver.commits (
    ...
    repo_id         String,
    source_system    String,          -- Matches bronze.raw_commits.source_system
...
| Layer | Action | Method |
| ------- | -------- | -------- |
| **Bronze** (`raw_people`) | Anonymize `_raw_json` for matching records | `ALTER TABLE ... UPDATE _raw_json = '{"redacted": true}' WHERE (source_system, employee_id) IN (SELECT source_system, alias_value FROM _purge_identifiers WHERE alias_type = 'employee_id')` |
| **Silver** (`commits_enriched`, etc.) | Set `person_id = 'REDACTED'` | `ALTER TABLE ... UPDATE person_id = 'REDACTED' WHERE person_id = :person_id` |
...
1. **Connector lag:** `now() - max(_synced_at)` per source_system in `bronze.raw_people`
...
CREATE TABLE bronze.raw_people_mt (
    tenant_id       UUID,
    -- ... existing columns ...
)
ENGINE = ReplacingMergeTree(_synced_at)
