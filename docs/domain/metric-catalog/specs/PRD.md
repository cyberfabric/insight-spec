...
| ~~`product-default` locks — can the product team ship a metric with a locked `product-default` row?~~ **Resolved in v1.7**: yes, but only through seed migrations (not at runtime), and **no runtime super-admin override exists**. Legitimate legal-override requests flow through backend code migrations that either lift the lock or remove the product-default row for the specific tenant — a slow but auditable path. This is intentional: a runtime super-admin escape hatch would erode the compliance guarantee that makes `is_locked` worth shipping. | Insight Product Team | Closed v1.7 |
